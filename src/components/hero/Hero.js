...
function Hero() {
  ...
  return (
    <motion.section
      ...
      whileInView="on"
      viewport={{ once: true }}
      className="h-screen grid items-center z-0"
    >
      <motion.div
        initial="off"
        whileInView="on"
        ...
        className="text-6xl sm:text-8xl text-gray-700 font-medium z-10 flex flex-col gap-6"
      >
      ...
      <motion.div
        variants={boxAnimate}
        className="absolute w-60 h-60 bg-yellow-300 shadow-[-12px_-12px] shadow-pink-500 -right-32 sm:right-[300px]"
      ></motion.div>
    </motion.section>
  );
}
